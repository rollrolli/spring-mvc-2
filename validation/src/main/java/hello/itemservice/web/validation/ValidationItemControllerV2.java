...
//    @PostMapping("/add")
    public String addItemV1(@ModelAttribute Item item, BindingResult bindingResult, RedirectAttributes redirectAttributes, Model model) {
        // BindingResult 는 꼭 커맨드객체 바로 다음에 파라미터로 선언해주어야 함
        // 검증 로직
        ...
        if (bindingResult.hasErrors()) {
            ...
            // BindingResult 는 자동으로 view 에 같이 넘어가므로 model.addAttribute 해줄 필요가 없다.
            return "validation/v2/addForm";
        }
    ...
    public String addItemV2(@ModelAttribute Item item, BindingResult bindingResult, RedirectAttributes redirectAttributes, Model model) {
        ...
        // 특정 필드가 아닌 복합 룰 검증
        if (item.getPrice() != null && item.getQuantity() != null) {
            if (item.getPrice() instanceof Integer && item.getQuantity() instanceof Integer) {
                int resultPrice = item.getPrice() * item.getQuantity();
                if (resultPrice < 10000) {
                    bindingResult.addError(new ObjectError("item", null, null,
                            "가격 * 수량의 합은 10,000 원 이상이어야 합니다. 현재 값 = " + resultPrice));
                }
            }
        }
